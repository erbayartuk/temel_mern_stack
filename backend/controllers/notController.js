...
const notlarGetir = async (req, res) => {
  const kullanici_id = req.kullanici._id;
  const notlar = await NotModel.find({ kullanici_id }).sort({ createdAt: -1 }); //en son eklenen ilk listelenmesi için sort kullanıldı
...
const notGuncelle = async (req, res) => {
  ...
  const not = await NotModel.findOneAndUpdate(
    { _id: id },
    { ...req.body },
    { new: true } //Gelen verinin güncel halini görmek için
  );
